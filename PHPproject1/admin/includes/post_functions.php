...
function getAllPosts($conn) {
        ...
        foreach ($posts as $post) {
                $post['author'] = getPostAuthorById($conn, $post['user_id']);
                array_push($final_posts, $post);
        }
        return $final_posts;
}
// get the author/username of a post
function getPostAuthorById($conn, $user_id) {
...
if (isset($_POST['create_post'])) { 
        createPost($conn, $_POST); 
}
// if user clicks the Edit post button
if (isset($_GET['edit-post'])) {
        $isEditingPost = true;
        $post_id = $_GET['edit-post'];
        editPost($conn, $post_id);
}
// if user clicks the update post button
if (isset($_POST['update_post'])) {
        updatePost($conn, $_POST);
}
...
-  Post functions
- - - - - - - - - - -*/
function createPost($conn, $request_values)
        {
                global $errors, $title, $featured_image, $topic_id, $body, $published;
                $title = esc($conn, $request_values['title']);
                $body = htmlentities(esc($conn, $request_values['body']));
                if (isset($request_values['topic_id'])) {
                        $topic_id = esc($conn, $request_values['topic_id']);
                }
                if (isset($request_values['publish'])) {
                        $published = esc($conn, $request_values['publish']);
                }
                ...
                var_dump($target);
                var_dump($_FILES['featured_image']['tmp_name']);
        ...
        * * * * * * * * * * * * * * * * * * * * * */
        function editPost($conn, $role_id)
        {
        ...
        function updatePost($conn, $request_values)
        {
                global $errors, $post_id, $title, $featured_image, $topic_id, $body, $published;

                $title = esc($conn, $request_values['title']);
                $body = esc($conn, $request_values['body']);
                $post_id = esc($conn, $request_values['post_id']);
                if (isset($request_values['topic_id'])) {
                        $topic_id = esc($conn, $request_values['topic_id']);
                }
                // create slug: if title is "The Storm Is Over", return "the-storm-is-over" as slug
...
if (isset($_GET['publish']) || isset($_GET['unpublish'])) {
    ...
    togglePublishPost($conn, $post_id, $message);
}
// delete blog post
function togglePublishPost($conn, $post_id, $message) {
